...
for i in $(ls /var/www/vhosts/); do
    ...
    /usr/sbin/logster -D --metric-prefix="servers.`hostname`.project.${sanitized_name}.apache.error_log" --output=graphite --graphite-host=graphite.everlution.com:2003 ApacheErrorLogster /var/www/vhosts/$i/statistics/logs/error.log
done;
